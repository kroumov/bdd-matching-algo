# -*- coding: utf-8 -*-
"""Resident-Hospital_Problem.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1aCiyyOC7Dhwgf8qAsppvhWi8wyidBzFh
...
lottery_num = lottery_numbers(dataframes[0], dataframes[1])

residency_position = dataframes[2].iloc[0, 1:].tolist()

print(residency_position)
...
df= matrix_prep(dataframes[0], residency_position, lottery_num)
...
print(hungarian_matrix_reduction(df))
matrix = hungarian_matrix_reduction(df)
...
print(minimum_lines(hungarian_matrix_reduction(df)))
...
#TO DO:
# find minimum covered lines and adjust weights
...
print(len(rows)+len(col))
print(rows, col)

subtract_min(reduced, rows, col)
...
print("Assignments (Doctor Index -> Residency Index):")
for doctor_idx, residency_idx in assignments:
    if residency_idx == -1:
        print(f"Doctor {doctor_idx} -> Unassigned")
    else:
        print(f"Doctor {doctor_idx} -> Slot {residency_idx}")
...
def main():
    ...
    if len(dataframes) < num_required_files:
        ...
        print("Make sure to import Doctor-Residency Rankings, Residency Positions, and Doctor Statistics.")
    ...
    # Residency positions list
    residency_position = df_positions.iloc[:, 0].tolist()

    # Convert rankings to cost matrix
    print("Processing rankings into a cost matrix...")
    cost_matrix = df_rankings.to_numpy()

    # Solve the assignment problem
    print("Solving assignment problem using the Hungarian algorithm...")
    assignments = assign_jobs(cost_matrix, residency_position)

    # Display assignments
    print("Organizing assignments...")
    organize_assignments(assignments, residency_position)
